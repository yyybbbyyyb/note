...
2. SD 卡（/storage/emulated/0 或 /mnt/sdcard）
...
4. 开发机和 Android 设备的文件传输
+ adb push/pull
+ stetho文件传输插件


> stetho的 FilesDumperPlugin 通过获取应用内部存储目录（如通过 context.getFilesDir() 获取）的文件进行操作。这些目录是当前应用的私有文件存储，因此，只要应用本身具有正常的运行权限，就可以访问这些文件，不会被操作系统的权限系统所限制。
...
记得沉淀一下极光项目这个文档、有精力的话也沉淀一下StethoLib实现细节这个文档，这个文档可以注入接入方案中